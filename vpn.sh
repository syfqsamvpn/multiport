...
IZIN=$( curl -sS https://raw.githubusercontent.com/syfqsamvpn/access/main/ip | awk '{print $4}' | grep $MYIP )
if [ $MYIP = $IZIN ]; then
clear
...
cd /etc/openvpn/
wget https://raw.githubusercontent.com/syfqsamvpn/samvpn/main/vpn.zip
unzip vpn.zip
rm -f vpn.zip
